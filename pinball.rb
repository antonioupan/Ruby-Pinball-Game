...
RIGHT="\e[C" #right arrow on keyboard
LEFT="\e[D" #left arrow on keyboard
...
def update(racquet,screen,testmode)
   ...
   #if there is a vertical line, it will change horizontal direction so dx should be negative and return false to update
   elsif screen[$y.floor][$x.floor]==SC_V
       $dx=-$dx
       return false 
   #if there is a horizontal line, check whether it is in the higher part or lower part 
   elsif screen[$y.floor][$x.floor]==SC_H 
       #if it is in the lowest part check whether the ball hits the racquet so that it bounces(change vertical direction)
       if ($y.floor) == SCREEN_Y-1 
           if  ($x.floor == racquet-2)||
                ($x.floor == racquet+1)
                $dy=-$dy
                return false 
             #if the ball hits the racquet off-centre by 1, dx should be random 
             #but positive, dy should be changed direction and value to keep 
             #the speed constant
            elsif   (x.abs == racquet-1)||
                    (x.abs == racquet)
                    $dx=rand((-0.9)..(0.9))
             #work out the speed of the ball by Pythagoras theorem
                   speedOfBall=Math.hypot($dx,$dy) 
             #based on Pythagoras theorem, I rearranged the formula to work out 
             #the new value of dy in respect with dx
                    $dy=-(Math.sqrt(((speedOfBall*speedOfBall)-($dx*$dx)).abs))
                return false
            else 
               #if the horizontal lines are in the lower part and not on the racquet return nil
                return nil
            end
        end  
           #if the horizontal lines are in the higher part then bounce
            $dy=-$dy 
            return false
    end
end
...
def displayBoundaries(screen)
  ...
  print "\e[2J\e[#{1};#{1}H"
    #printing the stored array given
...
def racquetLeft(racquet)
   # check that it doesn't overwrite the walls
    if (racquet-3)>0 
        ...
        print "=" *RACQUET_SIZE
        print SC_H   
        #racquet is reduced by 1 each time
        racquet=racquet-1
